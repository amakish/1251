<?php
		...
		foreach ($aRows[0] as $aRow){
			if ((strpos($aRow,'<th')===false)){
				preg_match_all("|<td(.*)</td>|U", $aRow, $aCells);
					...
					$oGoalieStat = new reg201011gstat;
	...
	for($i=1; $sPlyrNumXXOf != $sPlyrNumOfYYY; $i++) {
		$sStatsSummary = getGoalieStats("http://www.nhl.com/ice/playerstats.htm?fetchKey=20112ALLGAGALL&viewName=summary&sort=wins&pg=$i", "fgetRowsStatsSummary");
	...
	for($i=1; $sPlyrNumXXOf != $sPlyrNumOfYYY; $i++) {
		$sStatsBio = getGoalieStats("http://www.nhl.com/ice/playerstats.htm?fetchKey=20112ALLGAGALL&viewName=goalieBios&sort=player.birthCountryAbbrev&pg=$i", "fgetRowsStatsBio");
	...
	for($i=1; $sPlyrNumXXOf != $sPlyrNumOfYYY; $i++) {
		$sStatsRookie = getGoalieStats("http://www.nhl.com/ice/playerstats.htm?season=20102011&gameType=2&team=&position=G&country=&status=R&viewName=summary&pg=$i", "fgetRowsStatsRookie");
