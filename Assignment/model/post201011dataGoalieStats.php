<?php
		...
		foreach ($aRows[0] as $aRow){
			if ((strpos($aRow,'<th')===false)){
				preg_match_all("|<td(.*)</td>|U", $aRow, $aCells);
					...
					$oGoalieStat = new post201011gstat;
	...
	for($i=1; $sPlyrNumXXOf != $sPlyrNumOfYYY; $i++) {
		$sStatsSummary = getGoalieStats("http://www.nhl.com/ice/playerstats.htm?season=20102011&gameType=3&team=&position=G&country=&status=&viewName=summary&pg=$i", "fgetRowsStatsSummary");
	...
	for($i=1; $sPlyrNumXXOf != $sPlyrNumOfYYY; $i++) {
		$sStatsBio = getGoalieStats("http://www.nhl.com/ice/playerstats.htm?gameType=3&position=G&season=20102011&sort=player.birthCountryAbbrev&status=A&viewName=goalieBios&page=i", "fgetRowsStatsBio");
	...
	for($i=1; $sPlyrNumXXOf != $sPlyrNumOfYYY; $i++) {
		$sStatsRookie = getGoalieStats("http://www.nhl.com/ice/playerstats.htm?season=20102011&gameType=3&team=&position=G&country=&status=R&viewName=goalieBios&page=$i", "fgetRowsStatsRookie");
